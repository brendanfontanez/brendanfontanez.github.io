...
$.ajax(settings).done(function (states_current) {
    ...
    for (i = 0; i < JSON_FIELDS; i++) {
        ...
        if (STATES.includes(state['state'])) {
            var positive = state['positive'];
            var total_tested = state['posNeg'];
            
            var prop = positive / total_tested;
            if (prop > highest) {
                highest = prop;
            }
        }
    ...
    document.getElementById("item1").innerHTML = "0% to " + (threshold*100).toString().substring(0, 5) + "%";
    
    document.getElementById("item2").innerHTML = (threshold*100).toString().substring(0, 5) + "%" + " to " + (threshold*200).toString().substring(0, 5) + "%";
    
    document.getElementById("item3").innerHTML = (threshold*200).toString().substring(0, 5) + "%" + " to " + (threshold*300).toString().substring(0, 5) + "%";
    
    document.getElementById("item4").innerHTML = (threshold*300).toString().substring(0, 5) + "%" + " to " + (threshold*400).toString().substring(0, 5) + "%";
    
    document.getElementById("item5").innerHTML = (threshold*400).toString().substring(0, 5) + "%" + " to " + (threshold*500).toString().substring(0, 5) + "%";
});
